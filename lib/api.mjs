...
// A call that never answers (a node's door refusing this address, a node restarting) must not hang a watcher: every call times out.
const TIMEOUT_MS = Number(process.env.API_TIMEOUT_MS || 60000);
export class Node {
  constructor(url, name = url) { this.url = url.replace(/\/$/, ""); this.name = name; }
  async post(path, body, retried = false) {
    const res = await fetch(this.url + path, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(body), signal: AbortSignal.timeout(TIMEOUT_MS) });
    ...
    if (!res.ok || data.error) {
      ...
      if (!retried && body && body.collection && /needs every member local|read_parquet needs at least one file/.test(String(data.error || text))) { await ensureCollection(this, body.collection); return this.post(path, body, true); }
      throw new Error(`${this.name} ${path}: ${res.status} ${data.error || text.slice(0, 200)}`);
    }
    return data;
  }
  async get(path) { const res = await fetch(this.url + path, { signal: AbortSignal.timeout(TIMEOUT_MS) }); if (!res.ok) throw new Error(`${this.name} ${path}: ${res.status}`); return res.json(); }
...
// It waits a few seconds for the members to land, no more: a node may hold
// manifests of other collections whose members it never fetches.
export async function ensureCollection(node, collection) {
  ...
  for (let i = 0; i < 10; i++) {
    ...
    if (!files.some((f) => !f.local && f.name && (f.name.startsWith(collection + ".") || f.name.startsWith("unidatum-compact")) && !f.name.includes(".cursors."))) break;
    await sleep(300);
  }
  return 1;
}
