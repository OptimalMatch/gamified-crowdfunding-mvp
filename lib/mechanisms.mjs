...
// extra.match_cents: a sponsor's match raises the pool size and the winner
// count, never the entry weights (it is on the proof, so verifiers pass it too).
export function allocate(rule, entries, seed, beacon, extra = {}) {
  const pool = sum(entries, (e) => e.amount_cents) + (extra.match_cents || 0);
